...
void ND_NitscheIntegrator::AssembleElementMatrix(
    const mfem::FiniteElement&   el,
    mfem::ElementTransformation& Trans,
    mfem::DenseMatrix&           elmat)
{
    MFEM_ABORT(
        "ND_NitscheIntegrator::AssembleElementMatrix(): method is not "
...
{
    MFEM_ASSERT(
        Trans.Elem2No < 0, "support for interior faces is not implemented");
    ...
    for (int i = 0; i < ir->GetNPoints(); ++i)
    {
        ...
        // Sync face + element integration points. This ensures ip on the
        // element matches the face point orientation (important for tangential
        // fields).
        ...
        for (int l = 0; l < el1.GetDof(); l++)
            for (int k = 0; k < el1.GetDof(); k++)
            {
                ...
                mfem::Vector n_x_curl_u(dim), n_x_curl_v(dim), n_x_u(dim),
                    n_x_v(dim), n_x_u_x_n(dim), n_x_v_x_n(dim);
                ...
                elmat.Elem(l, k) +=
                    factor_ * weights[i] * area * (n_x_curl_u * v);
                elmat.Elem(l, k) +=
                    factor_ * theta_ * weights[i] * area * (u * n_x_curl_v);
                elmat.Elem(l, k) +=
                    factor_ * Cw_ / h * weights[i] * area * (n_x_u * n_x_v);
            }
    }
}
...
{
    MFEM_ABORT(
        "ND_NitscheIntegrator::AssembleRHSElementVect(): method is not "
        "implemented for this class");
}
    ...
    mfem::Vector&                     elvect)
{
    ...
    const mfem::IntegrationRule* ir = IntRule;
    ir                              = &mfem::IntRules.Get(
        static_cast<mfem::Geometry::Type>(Tr.FaceGeom), 2 * el.GetOrder() + 12);
